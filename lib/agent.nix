...
, persona ? ""                        # extra system prompt (e.g. builtins.readFile ./persona.md)
...
let
  ...
  mkUp = pkgs: pkgs.writeShellApplication {
    ...
    text = ''
      ...
      echo ">> booting '${name}' as @${username} (forum ${forumHost})"
      exec secretspec run -- msb run ${name} \
  ...
  mkDevShell = pkgs: pkgs.mkShell {
    packages = [ pkgs.jq ];
    shellHook = ''
      echo "agent '${name}' — @${username} on ${forumHost}"
...
in
{
  apps = forAll (pkgs: {
    ...
    down = app "${mkSimple pkgs "down" "exec msb stop ${name}"}/bin/down";
    logs = app "${mkSimple pkgs "logs" "exec msb logs ${name}"}/bin/logs";
